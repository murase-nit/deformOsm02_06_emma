...
/**
 * カテゴリ選択ウインドウにおける"ｼｮｯﾋﾟﾝｸﾞ"のパネル.
 * @author murase
 *
 */
public class MidiumCategory2Panel extends JPanel implements ActionListener{
	...
	public MidiumCategory2Panel(CategoryWindow aCategoryWindow){
		...
		for(int i=0; i<CategoryList.category1b2.length; i++){
			_category2CheckBoxArray.add(new JCheckBox(CategoryList.category1b2[i]));
	...
	public void initSelected(){
		for(int i=0; i<_category2CheckBoxArray.size(); i++){
			_category2CheckBoxArray.get(i).setSelected(true);
			_category2CheckMode.set(i, true);
		}
		_categoryWindow.setTabTitle(2, "All");
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource() == _category2CheckBoxArray.get(0)){	// "すべて"の項目がチェックされた.
			...
			_categoryWindow.setTabTitle(2, checkedNum);
			return;
		}
