 ...
 *
 */
public class GlueRoadXml {
	
	/** ストロークのID */
	public ArrayList<Integer> _strokeId = new ArrayList<>();
	/** ストロークの変形後の座標(xy座標(glueの左上が(0,0)) */
	public ArrayList<ArrayList<Point2D>> _strokePoint = new ArrayList<>();
	
	public GlueRoadXml(Point2D aCenterLngLat,int aFoucsZoomLevel, int aContextZoomLevel, int aGlueInnerRadius, int aGlueOuterRadius){
		_strokeId = new ArrayList<>();
		_strokePoint = new ArrayList<>();
		
		String uri = "http://133.68.13.112:8080/EmmaGlueMuraseOriginal/MainServlet?type=DrawGlue_v2" +
		...
		try {
			// DOMを使用するために新しいインスタンスを生成する.
		     ...
		     NodeList nodeList_1 = root.getElementsByTagName("oneStroke");
		     for(int i=0; i<nodeList_1.getLength(); i++){
		    	...
		    	_strokeId.add(Integer.parseInt(getChildVal(element, "selectedStrokeId").trim()));	// ストロークID取得.
